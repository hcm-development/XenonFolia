...
paperweight {
    ...
    val xenonfolia = forks.register("xenonfolia") {
        forks = luminol
        upstream.patchRepo("paperServer") {
            upstreamRepo = luminol.patchedRepo("paperServer")
            patchesDir = rootDirectory.dir("xenonfolia-server/paper-patches")
            outputDir = rootDirectory.dir("paper-server")
        }
        ...
        upstream.patchDir("luminolServer") {
            ...
            excludes = setOf("src/minecraft", "paper-patches", "minecraft-patches", "build.gradle.kts", "build.gradle.kts.patch")
            patchesDir = rootDirectory.dir("xenonfolia-server/luminol-patches")
            outputDir = rootDirectory.dir("luminol-server")
        }
    }

    activeFork = xenonfolia
...
dependencies {
    implementation(project(":xenonfolia-api")) // LightingLuminol
...
tasks.jar {
    manifest {
        ...
        attributes(
            ...
            "Specification-Title" to "XenonFolia",
            "Specification-Version" to project.version,
            "Specification-Vendor" to "HCM's Development",
            "Brand-Id" to "hcm:xenonfolia",
            "Brand-Name" to "XenonFolia",
...
fill {
    project("xenonfolia") // Luminol // XenonFolia
