...
val scanJar = tasks.register("scanJarForBadCalls", io.papermc.paperweight.tasks.ScanJarForBadCalls::class) {
    ...
    // File not found: build\libs\folia-server-1.21.1-R0.1-SNAPSHOT-dev.jar
}
...
val includeMappings = tasks.register<io.papermc.paperweight.tasks.IncludeMappings>("includeMappings") {
    inputJar.set(tasks.reobfJar.flatMap { it.outputJar })
...
tasks.reobfJar {
    ...
    finalizedBy(includeMappings)
}

tasks.register("copyReobfMappings", Copy::class) {
    group = "paperweight" // Folia
    description = "Copies the reobf mappings to the build directory" // Folia
    from(layout.projectDirectory.file("build-data/reobf-mappings-patch.tiny"))
    into(layout.buildDirectory.dir("tmp/mappings"))
    rename { "reobf-mappings-patch.tiny" }
}

tasks.assemble {
    dependsOn(includeMappings)
    dependsOn("copyReobfMappings")
}
...
tasks.serverJar {
    manifest {
        attributes("Add-Opens" to "java.base/java.lang java.base/java.io java.base/java.util java.base/java.util.jar java.base/java.util.zip java.base/java.util.concurrent java.base/sun.nio.ch java.base/java.net java.base/java.nio java.base/java.util.concurrent.atomic java.logging/java.util.logging java.base/java.lang.reflect java.base/javax.crypto java.base/java.nio.charset java.base/java.util.concurrent.locks java.base/sun.nio.fs java.base/java.lang.ref java.base/java.net java.base/java.nio.file java.base/java.security java.base/sun.nio.cs java.base/java.text java.sql/java.sql java.base/java.util.stream java.base/java.util.regex java.desktop/java.awt.font")
    }
}
...
tasks.register<JavaExec>("runBundler") {
    group = "paper"
    description = "Spin up a test server from the Mojang mapped bundler"
    ...
    mainClass.set("io.papermc.paperweight.patcher.bundler.PaperclipMain")
    systemProperty("bundlerFile", rootProject.tasks.named<io.papermc.paperweight.tasks.CreateBundlerJar>("createMojmapBundlerJar").flatMap { it.outputZip }.get().asFile.absolutePath)
}

tasks.register<JavaExec>("runReobfBundler") {
    group = "paper"
    description = "Spin up a test server from the reobf bundler"
    ...
    mainClass.set("io.papermc.paperweight.patcher.bundler.PaperclipMain")
    systemProperty("bundlerFile", rootProject.tasks.named<io.papermc.paperweight.tasks.CreateBundlerJar>("createReobfBundlerJar").flatMap { it.outputZip }.get().asFile.absolutePath)
}

tasks.register<JavaExec>("runPaperclip") {
    group = "paper"
    description = "Spin up a test server from the Mojang mapped Paperclip"
    ...
    systemProperty("paperclip.install", "true")
}

tasks.register<JavaExec>("runReobfPaperclip") {
    group = "paper"
    description = "Spin up a test server from the reobf Paperclip"
